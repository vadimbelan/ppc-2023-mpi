...
TEST(Monte_Carlo_Multiple_Integral_MPI, Test1) {
    ...
    double res_parall = multiple_integral_monte_carlo_parallel<1>(
                                f, region, rect, 300);
    if (rank == 0) {
        double res = multiple_integral_monte_carlo_sequential<1>(f, region, rect, 300);
        ASSERT_NEAR(res_parall, res, 100);
    }
}
...
TEST(Monte_Carlo_Multiple_Integral_MPI, Test_for_1) {
    ...
    double res_parall = multiple_integral_monte_carlo_parallel<1>(
                                f, region, rect, 30000);
    if (rank == 0) {
        double res = multiple_integral_monte_carlo_sequential<1>(f, region, rect, 30000);
        ASSERT_NEAR(res_parall, res, 4000);
    }
}
...
TEST(Monte_Carlo_Multiple_Integral_MPI, Test_for_3) {
    ...
    double res_parall = multiple_integral_monte_carlo_parallel<2>(
                                f, region, rect, 30000);
    if (rank == 0) {
        double res = multiple_integral_monte_carlo_sequential<2>(f, region, rect, 30000);
        ASSERT_NEAR(res_parall, res, 1000);
    }
}
...
TEST(Monte_Carlo_Multiple_Integral_MPI, Test_for_2) {
    ...
    double res_parall = multiple_integral_monte_carlo_parallel<2>(
                                f, region, rect, 3000);
    if (rank == 0) {
        double res = multiple_integral_monte_carlo_sequential<2>(f, region, rect, 3000);
        ASSERT_NEAR(res_parall, res, 10000);
    }
}
TEST(Monte_Carlo_Multiple_Integral_MPI, Test_for_4) {
    auto f = [](const array<double, 2> &x) -> double {
        return 1.0 - x[0]*x[0] - x[1]*x[1];
    };
    auto region = [](const array<double, 2> &x) -> bool {
    ...
    double res_parall = multiple_integral_monte_carlo_parallel<2>(
                                f, region, rect, 20000);
    if (rank == 0) {
        double res = multiple_integral_monte_carlo_sequential<2>(f, region, rect, 20000);
        ASSERT_NEAR(res_parall, res, 2000);
    }
}
