...
#include "task_3/savchuk_a_quick_simple_merge_sort/quicksort_simple_merge.h"
...
void parallel_quick_sort(int* arr, size_t n) {
    ...
    if (rank == 0) {
        ...
        for (int i = 1; i < ProcNum; ++i) {
            MPI_Send(&arr[my_count + other_count * (i - 1)],
        ...
        for (int i = 1; i < ProcNum; ++i) {
            MPI_Recv(&arr[my_count + other_count * (i - 1)],
                other_count, MPI_INT, i,
                456, MPI_COMM_WORLD, &status);
        }

        for (int i = 1; i < ProcNum; ++i) {
            merge(arr, my_count + other_count * (i - 1),
                other_count);
        }
    } else {
        MPI_Status status;
