...
#include "task_3/lebedeva_k_vertical_splitting/vertical_splitting.h"
#include <mpi.h>
#include <ctime>
#include <iostream>
#include <random>
...
int* getSequentialVertical_Splitting(const int* image, int width, int height) {
...
int* getParallelVertical_Splitting(const int* image, int width, int height) {
  int size, rank;
  ...
  if (delta) {
    ...
    local_matrix = getSequentialVertical_Splitting(local_matrix, temp + 2, height);
    ...
    if (rank == 0) {
      return getSequentialVertical_Splitting(image, width, height);
    } else {
      return nullptr;
    }
  }
