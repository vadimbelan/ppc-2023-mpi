...
#include "task_3/travin_m_global_opt/global_opt.h"
...
double getParameter_m(double parameterM, double parameterR) {
    return (parameterM == 0) ? 1 : parameterR * parameterM;
}

std::vector<RAndIndex> getParameterR(
    ...
    for (size_t i = 1; i < points.size(); i++) {
        double z1 = fun(points[i]);
        double z0 = fun(points[i - 1]);
        double pI = (points[i] - points[i - 1]);
        double currentR = parameter_m * pI +
            (z1 - z0) * (z1 - z0) / (parameter_m * pI) -
            2 * (z1 + z0);
...
double getNextX(
    ...
    return getNextX(points[indexOfMaxR - 1],
        points[indexOfMaxR],
        fun,
        parameter_m);
}
...
double globalSearchParallel(std::function<double(double)> fun,
    ...
    while (iterationCount < maxIterationCount) {
        ...
        if (rank == 0) {
            ...
            double m = getParameter_m(M, parameterR);
            auto R = getParameterR(sortedX, fun, m);

            std::sort(R.begin(), R.end(),
                [](const RAndIndex& p1, const RAndIndex& p2) {
                return p1.first > p2.first;
                });

            int indexOfMaxR = R.front().second;
            inputBuffer = { sortedX[indexOfMaxR - 1], sortedX[indexOfMaxR], m };

            workedProcessCount = std::min(static_cast<size_t>(count), R.size());
            for (int i = 1; i < workedProcessCount; i++) {
                RAndIndex r = R[i];
                std::vector<double> outputBuffer =
                { sortedX[r.second - 1], sortedX[r.second],  m };
                MPI_Send(outputBuffer.data(), 3,
        ...
        if (rank != 0 && rank < workedProcessCount) {
            MPI_Status status;
            MPI_Recv(inputBuffer.data(), 3, MPI_DOUBLE,
        ...
        if (rank < workedProcessCount) {
            double nextX = getNextX(inputBuffer[0],
                inputBuffer[1],
                fun,
                inputBuffer[2]);
            ...
            if (rank != 0) {
                ...
                for (int i = 1; i < workedProcessCount; i++) {
                    double inputNextX;
                    MPI_Status status;
                    MPI_Recv(&inputNextX, 1, MPI_DOUBLE, i,
                    ...
                    auto upperBound = std::upper_bound(sortedX.begin(),
                        sortedX.end(), inputNextX);
