...
TEST(rectangle_integration_parallel, identity_function) {
    ...
    double parallelVersion = RectangleIntegrateParallel(f, lowerBound, upperBound, step);

    if (world.rank() == 0) {
        double sequentialVersion = RectangleIntegrateSequential(f, lowerBound, upperBound, step);
