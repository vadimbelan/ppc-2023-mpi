...
#include "task_1/balyasov_num_of_alternations_signs/num_of_alternations_signs.h"
...
int par_num_of_alternations_signs(const std::vector<int>& vec) {
  ...
  for (int i = 0; i < world_size; ++i) {
    if (i < remain) ++count_send[i];
    displs[i] = i == 0 ? 0 : displs[i - 1] + count_send[i - 1];
    if (count_send[i]) ++worked;
  }

  for (int i = 0; i < worked; ++i) ++count_send[i];
