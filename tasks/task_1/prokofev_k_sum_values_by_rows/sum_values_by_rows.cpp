...
#include "task_1/prokofev_k_sum_values_by_rows/sum_values_by_rows.hpp"
...
std::vector<int> ParallSumValuesByRows(std::vector<int>& matrix, int n, int m)
{
	...
	MPI_Scatter(matrix.data(), rowsForOneProc * m, MPI_INT, localMatrix.data(), rowsForOneProc * m, MPI_INT, 0, MPI_COMM_WORLD);
