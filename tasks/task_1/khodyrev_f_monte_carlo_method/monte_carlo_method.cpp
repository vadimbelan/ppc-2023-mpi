...
double upper_limit, std::function<double(double)> func) {
    ...
    std::uniform_real_distribution<double> distribution(lower_limit,
    upper_limit);
    for (int i = 0; i < num_points; i++) {
        double x = distribution(generator);
        double y = func(x);
        unsigned int seed = 42;
        if (y > static_cast<double>(rand_r(&seed)) / RAND_MAX) {
            points_inside++;
        }
    }
...
double upper_limit, std::function<double(double)> func) {
    ...
    MPI_Reduce(&local_points_inside, &points_inside, 1, MPI_INT, MPI_SUM,
    0, MPI_COMM_WORLD);
    if (rank == 0) {
        double area = (upper_limit - lower_limit) * 1.0 *
        points_inside / num_points;
