...
#include "task_2/nogin_d_scatter/algo.h"
...
void MyScatter(const void* sendbuf, int sendcount, MPI_Datatype sendtype,
    ...
    if (rank == root) {
        for (int i = 0; i < size; ++i) {
            if (i != root) {
                MPI_Send(static_cast<const char*>(sendbuf) + i * sendcount * typeSize, sendcount, sendtype,
                         i, 0, comm);
            } else {
                std::copy(static_cast<const char*>(sendbuf) + i * sendcount * typeSize,
                          static_cast<const char*>(sendbuf) + (i + 1) * sendcount * typeSize,
